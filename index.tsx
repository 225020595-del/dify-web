...
export default function Home() {
  // 当前激活的功能模块: 'resume' | 'jd' | 'recruit'
  const [activeModule, setActiveModule] = useState<'resume' | 'jd' | 'recruit'>('resume')
  ...
  const [scoreData, setScoreData] = useState<{
    ...
    analysis: number
    potential: number
  }>({ total: 0, strengths: 0, gaps: 0, analysis: 0, potential: 0 })
  ...
  const [recruitResult, setRecruitResult] = useState<QueryResult | null>(null)
  ...
  const analyzeResume = async () => {
    ...
    try {
      ...
      // 提取评分数据 - 精确匹配详细报告中每个章节的实际分数
      let totalScore = 0
      let scores = { strengths: 0, gaps: 0, analysis: 0, potential: 0 }
      
      // 匹配 "得分：7.8/10" 格式
      const scorePattern = /得分[：:]\s*(\d+(?:\.\d+)?)(?:\/10)?/g
      const matches = [...fullResult.matchAll(scorePattern)]
      
      // 按出现顺序提取分数
      if (matches.length >= 1) totalScore = parseFloat(matches[0][1])
      if (matches.length >= 2) scores.strengths = parseFloat(matches[1][1])
      if (matches.length >= 3) scores.gaps = parseFloat(matches[2][1])
      if (matches.length >= 4) scores.analysis = parseFloat(matches[3][1])
      if (matches.length >= 5) scores.potential = parseFloat(matches[4][1])
      
      console.log('提取的分数:', { totalScore, ...scores })
          ...
          {result && (
            <>
              ...
              <div className="relative z-10 mb-8">
                {/* 总体匹配得分 - 独立显示 */}
                <div className="mb-6">
                  <div className="bg-gradient-to-br from-teal-500/20 to-cyan-700/20 backdrop-blur-sm rounded-xl p-6 border border-teal-500/30 hover:scale-105 transition-transform">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-semibold text-teal-300">总体匹配得分</h3>
                      </div>
                      <div className="text-3xl font-bold text-teal-300">{scoreData.total.toFixed(1)}</div>
                    </div>
                    <div className="w-full bg-slate-800/50 rounded-full h-2">
                      <div 
                        className="bg-gradient-to-r from-teal-500 to-cyan-400 h-2 rounded-full transition-all duration-1000"
                        style={{ width: `${(scoreData.total / 10) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* 关键优势 */}
                  <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-700/20 backdrop-blur-sm rounded-xl p-6 border border-emerald-500/30 hover:scale-105 transition-transform">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-semibold text-emerald-300">关键优势</h3>
                      </div>
                      <div className="text-3xl font-bold text-emerald-300">{scoreData.strengths.toFixed(1)}</div>
                    </div>
                    <div className="w-full bg-slate-800/50 rounded-full h-2">
                      <div 
                        className="bg-gradient-to-r from-emerald-500 to-emerald-400 h-2 rounded-full transition-all duration-1000"
                        style={{ width: `${(scoreData.strengths / 10) * 100}%` }}
                      />
                    </div>
                  </div>

                  {/* 潜在差距 */}
                  <div className="bg-gradient-to-br from-amber-500/20 to-amber-700/20 backdrop-blur-sm rounded-xl p-6 border border-amber-500/30 hover:scale-105 transition-transform">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-semibold text-amber-300">潜在差距</h3>
                      </div>
                      <div className="text-3xl font-bold text-amber-300">{scoreData.gaps.toFixed(1)}</div>
                    </div>
                    <div className="w-full bg-slate-800/50 rounded-full h-2">
                      <div 
                        className="bg-gradient-to-r from-amber-500 to-amber-400 h-2 rounded-full transition-all duration-1000"
                        style={{ width: `${(scoreData.gaps / 10) * 100}%` }}
                      />
                    </div>
                  </div>

                  {/* 详细分析 */}
                  <div className="bg-gradient-to-br from-blue-500/20 to-blue-700/20 backdrop-blur-sm rounded-xl p-6 border border-blue-500/30 hover:scale-105 transition-transform">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-semibold text-blue-300">详细分析</h3>
                      </div>
                      <div className="text-3xl font-bold text-blue-300">{scoreData.analysis.toFixed(1)}</div>
                    </div>
                    <div className="w-full bg-slate-800/50 rounded-full h-2">
                      <div 
                        className="bg-gradient-to-r from-blue-500 to-blue-400 h-2 rounded-full transition-all duration-1000"
                        style={{ width: `${(scoreData.analysis / 10) * 100}%` }}
                      />
                    </div>
                  </div>

                  {/* 成长潜力 */}
                  <div className="bg-gradient-to-br from-purple-500/20 to-purple-700/20 backdrop-blur-sm rounded-xl p-6 border border-purple-500/30 hover:scale-105 transition-transform">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-semibold text-purple-300">成长潜力</h3>
                      </div>
                      <div className="text-3xl font-bold text-purple-300">{scoreData.potential.toFixed(1)}</div>
                    </div>
                    <div className="w-full bg-slate-800/50 rounded-full h-2">
                      <div 
                        className="bg-gradient-to-r from-purple-500 to-purple-400 h-2 rounded-full transition-all duration-1000"
                        style={{ width: `${(scoreData.potential / 10) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/50 rounded-xl p-6 border border-slate-700/50">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xl font-semibold text-white">详细AI分析报告</span>
                    </div>
                    <button
                      onClick={() => {
